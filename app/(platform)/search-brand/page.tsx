...
function FilterOption({ option }: any) {
  ...
  return (
    <div
      className={`w-full flex flex-col items-center text-center py-3 height-auto cursor-pointer`}
      onClick={() => setExpanded(!expanded)}
    >
      <div className="flex w-full">
        <div className="flex-1 text-left">company filter option</div>
        <div className="select-none">{expanded ? "-" : "+"}</div>
      </div>
      {expanded && <div className="mt-4">option</div>}
    </div>
  );
}
      ...
      <div className="bg-gray-200 p-6 rounded-lg shadow-md flex flex-col gap-4">
        ...
        <div className="divide-y divide-gray-300 flex flex-col">
          {[1, 2, 3, 4].map((option, index) => (
            <FilterOption option={option} key={index} />
          ))}
        </div>
      </div>
