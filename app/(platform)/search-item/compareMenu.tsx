...
export default function CompareMenu({ selectedItems = [], setSelectedItems, isDragging: isDraggingMainPage, onClose }: { selectedItems?: any[], setSelectedItems: any, isDragging: boolean, onClose: () => void }) {
    const maxHeight = 435;
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        try {
            ...
            const isAlreadySelected = selectedItems.some((selected: any) => selected.id === item.id);
    ...
    if (isDraggingMainPage) {
        ...
        >
            <div className={`border-2 border-dashed rounded-lg w-full h-full text-center flex flex-col items-center justify-center transition-colors ${
                isDragOver 
    ...
    return (
        ...
        <div 
            ...
            style={{ 
                boxShadow: '0 -4px 8px 0 rgba(0, 0, 0, 0.25)',
                height: `${height}px`,
                transition: isDragging ? 'height 0.1s ease' : 'height 0.2s ease, transform 0.3s ease-out, opacity 0.3s ease-out'
            }}
        >
                ...
                <div className="flex">
                    <div className="grid grid-cols-4 gap-4">
                        {selectedItems.slice(0, 4).map((item: any, index: number) => (
                            <div className="flex flex-col items-start lg:min-w-[200px] lg:min-h-[300px]" key={index}>
                                <div className="flex-1 w-full">
                                    <ClothingItem item={item} />
                                </div>
                                <button className="mt-2 bg-gray-200 hover:bg-gray-300 text-black px-3 py-2 rounded text-sm transition-colors"
                    ...
                    {selectedItems.length > 0 && (
                        <div className="flex-1 flex justify-end items-end">
                            <button onClick={() => setLargeCompareOpen(true)} className="bg-black hover:bg-gray-800 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                                Compare {selectedItems.length} Item{selectedItems.length !== 1 ? 's' : ''}
                            </button>
                        </div>
                    )}
                    </div>
        ...
        {largeCompareOpen && (
            <CompareMenuLarge selectedItems={selectedItems} onClose={() => setLargeCompareOpen(false)} />
        )}
        </>
    )
}